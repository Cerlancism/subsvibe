...
The matrix above describes SubsVibe's **live** path (its main differentiator). SubsVibe also has a **file mode** (`--input <file>`) that transcribes an existing audio or video file to an `.srt` — VAD segmentation, transcription, and SRT timing, but no LLM refinement or translation (those are live-only).
